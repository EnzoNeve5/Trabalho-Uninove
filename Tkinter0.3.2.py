...
class Application():
    def __init__(self):
        ...
        self.conn = sqlite3.connect('ong.db')
    ...
    def criar_tabelas(self):
        # Criação da tabela de usuários e doações no banco de dados
        self.cursor.execute('''
    ...
    def verificar_login(self):
        # Função para verificar se o email e senha inseridos no login correspondem a um usuário cadastrado.
    ...
    def salvar_doacao(self):
        # Coleta os dados inseridos na janela de doação e insere no banco de dados.
        ...
        # Converte o valor para float para ser armazenado corretamente
        try:
            valor = float(valor)
        except ValueError:
        ...
        self.cursor.execute('''
            INSERT INTO doacoes (nome_instituicao, valor, chave_pix)
            VALUES (?, ?, ?)
        ''', (nome_instituicao, valor, chave_pix))
        self.conn.commit()
